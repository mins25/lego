...
const linkWithHighestReduction = "https://www.dealabs.com/bons-plans/le-croiseur-dassaut-de-classe-venator-de-la-republique-ucs-lego-star-wars-75367-3009673";
console.log('highest reduction', linkWithHighestReduction);
...
function sortDealsByPrice(dealsList) {
    return dealsList.slice().sort((a, b) => a.price - b.price);
}
...
function sortDealsByDate(dealsList) {
    return dealsList.slice().sort((a, b) => new Date(a.published) - new Date(b.published));
}
...
// 2. 
console.log('Deals with a discount btw 50% and 75% :', filteredDeals);
...
function calculateStatistics(listings) {
  // 1
  const prices = listings.map(item => parseFloat(item.price)).sort((a, b) => a - b);
...
const stats = calculateStatistics(VINTED);
console.log(`Moyenne: ${stats.average.toFixed(2)}€`);
console.log(`p5: ${stats.p5.toFixed(2)}€`);
